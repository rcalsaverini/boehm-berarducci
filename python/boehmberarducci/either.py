...
class Either(object):
    ...
    def __eq__(self, other):
        def is_eq_left(l1):
            return other(lambda l2: l1 == l2, const(False))

        def is_eq_right(r1):
            return other(const(False), lambda r2: r1 == r2)

        return self(is_eq_left, is_eq_right)


def left(x):
    return Either(lambda left_fun, _: left_fun(x))


def right(x):
    return Either(lambda _, right_fun: right_fun(x))
